...
local function isPointInside(instance, point)
...
local function positionPopup(window, popup, anchor, yOffset)
	...
	local maxX = math.max(4, window.Root.AbsoluteSize.X - popupSize.X - 4)
	local maxY = math.max(4, window.Root.AbsoluteSize.Y - popupSize.Y - 4)
...
function Window:AddTab(name)
	local tabFrame = frame(self.TabBar, "Tab_" .. name, UDim2.fromOffset(0, 0), UDim2.fromOffset(1, TAB_HEIGHT), Theme.Tab, 107)
	...
	local page = frame(self.Content, "Page_" .. name, UDim2.fromOffset(0, 0), UDim2.new(1, 0, 1, 0), Theme.Body, 106)
...
function Tab:AddGroupbox(title, position, size)
	local box = frame(self.Page, "Groupbox_" .. title:gsub("%W+", "_"), position, size, Theme.Body, 108)
...
function Groupbox:AddSubTabs(names, activeIndex, position, size)
	...
	for index, name in ipairs(names) do
		...
		local itemFrame = frame(holder, "SubTab_" .. name, UDim2.fromOffset((index - 1) * baseWidth, 0), UDim2.fromOffset(width, size.Y.Offset), Theme.Tab, 117)
...
function Groupbox:AddText(text, x, y, width)
	return label(self.Frame, "Text_" .. text:gsub("%W+", "_"), text, UDim2.fromOffset(x, y), UDim2.fromOffset(width or 180, 14), Theme.Text, Enum.TextXAlignment.Left, 118)
end

function Groupbox:AddCheckbox(text, x, y, checked, colorBox, callback)
	local row = frame(self.Frame, "Checkbox_" .. text:gsub("%W+", "_"), UDim2.fromOffset(x, y), UDim2.fromOffset(210, 14), Theme.Body, 118)
...
function Groupbox:AddDropdown(text, x, y, width, values, default, callback)
	...
	local control = makeControl(self.Frame, "Dropdown_" .. text:gsub("%W+", "_"), UDim2.fromOffset(x, y + 14), UDim2.fromOffset(width, 19), 118)
	...
	local menu = frame(self.Window.PopupLayer, "Menu_" .. text:gsub("%W+", "_"), UDim2.fromOffset(0, 0), UDim2.fromOffset(width, menuHeight), Theme.DropdownMenu, 520)
	menu.Visible = false
...
function Dropdown:SetOpen(open)
	...
	if open then
		...
		local controlPosition = self.Frame.AbsolutePosition
		local rootPosition = self.Window.Root.AbsolutePosition
		self.Menu.Position = UDim2.fromOffset(controlPosition.X - rootPosition.X, controlPosition.Y - rootPosition.Y + self.Frame.AbsoluteSize.Y + 1)
...
function Groupbox:AddKeyPicker(text, x, y, width, default, callback)
	...
	local control = makeControl(self.Frame, "KeyPicker_" .. text:gsub("%W+", "_"), UDim2.fromOffset(x, y + 14), UDim2.fromOffset(width, 19), 118)
...
function Groupbox:AddSlider(text, x, y, width, ratio, valueText, callback)
	local holder = frame(self.Frame, "Slider_" .. text:gsub("%W+", "_"), UDim2.fromOffset(x, y), UDim2.fromOffset(width, 28), Theme.Body, 118)
...
function Groupbox:AddButton(text, x, y, width, height, callback)
	local control = makeControl(self.Frame, "Button_" .. text:gsub("%W+", "_"), UDim2.fromOffset(x, y), UDim2.fromOffset(width, height or 20), 118)
...
function Groupbox:AddColorPicker(text, x, y, width, default, callback)
	...
	local holder = frame(self.Frame, "ColorPicker_" .. text:gsub("%W+", "_"), UDim2.fromOffset(x, y), UDim2.fromOffset(width, 16), Theme.Body, 118)
	holder.BackgroundTransparency = 1
	...
	local popup = frame(self.Window.PopupLayer, "ColorMenu_" .. text:gsub("%W+", "_"), UDim2.fromOffset(0, 0), UDim2.fromOffset(popupWidth, popupHeight), Theme.DropdownMenu, 540)
	popup.Visible = false
	stroke(popup, Color3.fromRGB(24, 23, 29), 1)

	label(popup, "Title", text, UDim2.fromOffset(7, 3), UDim2.new(1, -14, 0, 16), Theme.Text, Enum.TextXAlignment.Left, 545)
	...
	self.Window:_signal(UserInputService.InputBegan, function(input, gameProcessed)
		if gameProcessed or input.UserInputType ~= Enum.UserInputType.MouseButton1 or not colorPicker.Open then
			return
		end

		if not isPointInside(colorPicker.Instance, input.Position) and not isPointInside(colorPicker.Popup, input.Position) then
...
function ColorPicker:SetOpen(open)
	...
	if open then
		...
		positionPopup(self.Window, self.Popup, self.Instance, 2)
	elseif self.DraggingMap or self.DraggingHue then
